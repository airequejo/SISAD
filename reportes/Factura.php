...
function fact_dev( $libelle, $num )
{
    ...
    $mid = ($r1 + $r2 ) / 2;
    if($libelle==1){
    	$texto=utf8_decode("FACTURA ELECTRONICA F");
    }
    else{
		$texto=utf8_decode("BOLETA ELECTRONICA B");
    }
    $texte  = $texto  . $num;    
    $szfont = 12;
    $loop   = 0;
...
function addDate( $date )
{
	...
	$this->Cell(10,5, "Fecha de Emisi�n", 0, 0, "C");
...
function addCadreTVAs($monto)
{
	...
	$r1  = 10;
	$r2  = $r1 + 130;
	$y1  = $this->h - 24; //Distancia desde el objeto de arriba
	$y2  = $y1+20;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 2.5, 'D');
	...
	$this->MultiCell(100,4, "Guarda tu comprobante. Representaci�n impresa del Comprobante de Venta Electr�nico, puede ser consultado en coba.esapsis.com");
	//$this->SetX( $r1+29 );
...
function addCadreEurosFrancs($impuesto)
{
	...
	$this->SetXY( $r1, $y1+6 );
	$this->Cell(20,4, "Op. Exonerada:", 0, 0, "L");
	$this->SetXY( $r1, $y1+8 );
	$this->Cell(20,4, "Op. Inafecta:", 0, 0, "L");
	$this->SetXY( $r1, $y1+10 );
	$this->Cell(20,4, "Op. Gravada:", 0, 0, "L");
	$this->SetXY( $r1, $y1+12 );
	$this->Cell(20,4, "Op. Gratuita:", 0, 0, "L");
	$this->SetXY( $r1, $y1+14 );
	$this->Cell(20,4, "Importe a Pagar:", 0, 0, "L");
}
